...
class CFrontControl{
	...
	public function route(CMessage $msg){
		...
		if (!class_exists($msg -> getControl())) {
			$GLOBALS['UXLL_CONFIG']['route_table'] = require_once(ROOT.'config/routeTable.php');
			if(
			...
			}elseif(file_exists(MODEL.$msg -> getControl().'/'.$msg -> getControl().'.php')){
				require_once(MODEL.$msg -> getControl().'/'.$msg -> getControl().'.php');
				if (!class_exists($msg -> getControl())){
					$this -> routeErr();
					return false;
				}
		...
		if ($rc -> implementsInterface('IController')) {
			if(!$rc -> hasMethod(C('action_not_found')) && !$rc -> hasMethod($msg -> getAction())){
				...
				//经过的试验，知道就是调用controller 的method方法
				return false;
			}else{
